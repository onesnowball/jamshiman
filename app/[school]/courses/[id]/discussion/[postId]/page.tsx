...
export default async function CourseDiscussionPostPage({
  ...
  params: { school: string; id: string; postId: string }
}) {
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 py-8 page-enter space-y-6">
      <div className="flex items-center gap-2 text-sm text-gray-400">
        <Link href={`/${params.school}/courses`} className="hover:text-gray-600">Courses</Link>
        <span>/</span>
        <Link href={base} className="hover:text-gray-600">{course.code}</Link>
        <span>/</span>
        <Link href={`${base}?tab=discussion`} className="hover:text-gray-600">Discussion</Link>
      </div>
